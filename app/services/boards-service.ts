...
import type { BoardResponse, BoardsResponse, BoardTasksResponse } from "./interfaces/boards-service.interface";
...
export const getBoard = async (slug: string, cookie?: string) => {
  return await api.get(`boards/${slug}`, { cookie })
...
export const getBoardTasks = async (slug: string, cookie?: string) => {
  return await api.get(`tasks/board/${slug}`, { cookie })
